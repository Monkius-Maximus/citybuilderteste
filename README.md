...
- **Biblioteca de Cidades (M1+M2)** — gerenciador CRUD estilo TheoTown/SC4: save atômico, renomear sem carregar o mundo, duplicar, lixeira `.trash/` não destrutiva, autosave com rotação de 5 slots, **founding codes** compartilháveis (legível + base32, verificados) e **export/import** `.polispack` com checagem de integridade + CLI `list/export/import` ([plano completo](docs/plans/city-management.md)).
