...
- **Gerenciamento de Cidades (completo)** — gerenciador CRUD estilo TheoTown/SC4: save atômico, renomear sem carregar o mundo, duplicar, lixeira `.trash/` não destrutiva, autosave com rotação de 5 slots, **founding codes** compartilháveis (legível + base32, verificados), **export/import** `.polispack` com checagem de integridade, **thumbnails** 64×44 no save (v3, lendo v2..v3), CLI `list/export/import` e o `GameHost` que liga os menus (`GameShell`) à biblioteca e ao ciclo da simulação ([plano completo](docs/plans/city-management.md)).
